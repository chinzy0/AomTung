...
class AddIncomeViewModel : ViewModel() {
    ...
    fun incomeDeleteClick() {
        onClick.value = "incomeDeleteClick"
    }
    ...
    fun deleteIncome(
        ...
        clickCallback: ((UpdateIncome) -> Unit)) {
        val jsonContent = JSONObject()
            .put("income_id", income_id).toString()

        val client = OkHttpClient()
        val requestBody = jsonContent.toRequestBody("application/json".toMediaType())
        val request = Request.Builder()
            .url("http://zaserzafear.thddns.net:9973/api/Incomes/DeleteIncome?id=${income_id}")
            .delete(requestBody)
            .build()
